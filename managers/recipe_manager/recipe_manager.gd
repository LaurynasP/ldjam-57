...
func is_resource_in_recipes(resource: String, available_recipes: Array[Recipe]) -> bool:
...
func is_resource_in_recipe(resource: String, recipe: Recipe) -> bool:
	return recipe.ingredients.any(func(i: Item): return i.name == resource)

func get_related_recipes(resource: String, available_recipes: Array[Recipe]) -> Dictionary[String, Recipe]:
	var result: Dictionary[String, Recipe] = {}
	for recipe in available_recipes:
		if is_resource_in_recipe(resource, recipe):
